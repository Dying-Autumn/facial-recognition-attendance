        ...
        switch (id) {
                ...
                content = `
                    <div class="card">
                        <div class="card-header">
                            ...
                            <button class="btn" onclick="loadRoles()">🔄 刷新</button>
                        </div>
                        <div class="card-body">
                            <div class="table-container">
                                <table>
                ...
                content = `
                    <div class="card">
                        <div class="card-header">
                            ...
                            <button class="btn btn-accent" onclick="addFunction()">➕ 添加功能</button>
                            <button class="btn" onclick="loadFunctions()">🔄 刷新</button>
                        </div>
                        <div class="card-body">
                            <div class="table-container">
                                <table>
            ...
            case 'role-assignment':
                content = `
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">角色分配管理</div>
                            <button class="btn" onclick="loadRoleAssignments()">🔄 刷新</button>
                        </div>
                        <div class="card-body">
                            <div class="form-group">
                                <label>选择角色</label>
                                <select id="role-select-assignment" onchange="loadUsersByRole()">
                            ...
                            <div class="table-container" style="margin-top: 20px;">
                                <table>
                                    ...
                                    <tbody id="user-role-table-body">
                                        <tr>
                                            <td colspan="6" style="text-align: center;">请先选择角色</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                ...
                `;
                setTimeout(initRoleAssignmentPage, 100);
                break;
            case 'permission-assignment':
                content = `
                    <div class="card">
                        <div class="card-header">
                            <div class="card-title">权限分配管理</div>
                            <button class="btn" onclick="loadPermissionAssignments()">🔄 刷新</button>
                        </div>
                        <div class="card-body">
                            <div class="form-group">
                                <label>选择角色</label>
                                <select id="role-select-permission" onchange="loadPermissionsByRole()">
                                    <option value="">请选择角色...</option>
                                </select>
                            </div>
                            <div id="permission-assignment-area" style="margin-top: 20px;">
                                <p style="text-align: center; color: #999;">请先选择角色</p>
                            </div>
                        </div>
                    </div>
                `;
                setTimeout(initPermissionAssignmentPage, 100);
                break;
            default:
                content = `
                    <div class="card">
...
async function loadRoles() {
    ...
    try {
        ...
        if (roles.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center;">暂无角色数据</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = roles.map(role => `
            <tr>
                ...
                <td><span class="badge ${role.status === 'ACTIVE' ? 'badge-success' : 'badge-warning'}">${role.status || 'INACTIVE'}</span></td>
                <td>${role.createdTime ? new Date(role.createdTime).toLocaleString('zh-CN') : ''}</td>
                <td>
                    <div class="btn-group">
                        <button class="btn" onclick="editRole(${role.roleId}, '${role.roleName || ''}', '${(role.description || '').replace(/'/g, "\\'")}', '${role.status || 'INACTIVE'}')">✏️ 编辑</button>
                        <button class="btn btn-danger" onclick="deleteRole(${role.roleId}, '${role.roleName || ''}')">🗑️ 删除</button>
                    </div>
                </td>
            </tr>
        `).join('');
        ...
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; color: red;">加载失败，请检查后端服务</td></tr>';
    }
}
...
function addRole() {
    Modal.form({
        ...
        fields: [
            ...
            { id: 'description', name: 'description', label: '描述', type: 'textarea', required: false },
            { id: 'status', name: 'status', label: '状态', type: 'select', required: true, 
              options: [
...
// 编辑角色
function editRole(roleId, roleName, description, status) {
    Modal.form({
        title: '✏️ 编辑角色',
        fields: [
            { id: 'roleName', name: 'roleName', label: '角色名称', type: 'text', required: true, value: roleName },
            { id: 'description', name: 'description', label: '描述', type: 'textarea', required: false, value: description },
            { id: 'status', name: 'status', label: '状态', type: 'select', required: true, value: status,
              options: [
                  { value: 'ACTIVE', label: '活跃' },
                  { value: 'INACTIVE', label: '非活跃' }
              ]}
...
async function loadFunctions() {
    ...
    try {
        const functions = await FunctionAPI.getAll();
        if (functions.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align: center;">暂无功能数据</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = functions.map(func => `
            <tr>
                ...
                <td>${func.functionPath || ''}</td>
                <td>${func.subsystem || ''}</td>
                <td>${func.description || ''}</td>
                <td><span class="badge ${func.status === 'ACTIVE' ? 'badge-success' : 'badge-warning'}">${func.status || 'INACTIVE'}</span></td>
                <td>
                    <div class="btn-group">
                        <button class="btn" onclick="editFunction(${func.functionId}, '${(func.functionName || '').replace(/'/g, "\\'")}', '${(func.functionPath || '').replace(/'/g, "\\'")}', '${(func.subsystem || '').replace(/'/g, "\\'")}', '${(func.description || '').replace(/'/g, "\\'")}', '${func.status || 'INACTIVE'}')">✏️ 编辑</button>
                        <button class="btn btn-danger" onclick="deleteFunction(${func.functionId}, '${(func.functionName || '').replace(/'/g, "\\'")}')">🗑️ 删除</button>
                    </div>
                </td>
            </tr>
        `).join('');
        ...
        tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; color: red;">加载失败，请检查后端服务</td></tr>';
    }
}
...
function addFunction() {
    Modal.form({
        title: '➕ 添加功能',
        fields: [
            { id: 'functionName', name: 'functionName', label: '功能名称', type: 'text', required: true },
            { id: 'functionPath', name: 'functionPath', label: '功能路径', type: 'text', required: true },
            { id: 'subsystem', name: 'subsystem', label: '子系统', type: 'text', required: true },
            { id: 'description', name: 'description', label: '描述', type: 'textarea', required: false },
            { id: 'status', name: 'status', label: '状态', type: 'select', required: true,
              options: [
                  { value: 'ACTIVE', label: '活跃' },
        ...
        onSubmit: (formData) => {
            const functionEntity = {
                ...
                functionPath: formData.functionPath,
                subsystem: formData.subsystem,
                description: formData.description,
                status: formData.status || 'ACTIVE'
            };
            
            FunctionAPI.create(functionEntity)
                .then(() => {
...
// 编辑功能
function editFunction(functionId, functionName, functionPath, subsystem, description, status) {
    Modal.form({
        title: '✏️ 编辑功能',
        fields: [
            { id: 'functionName', name: 'functionName', label: '功能名称', type: 'text', required: true, value: functionName },
            { id: 'functionPath', name: 'functionPath', label: '功能路径', type: 'text', required: true, value: functionPath },
            { id: 'subsystem', name: 'subsystem', label: '子系统', type: 'text', required: true, value: subsystem },
            { id: 'description', name: 'description', label: '描述', type: 'textarea', required: false, value: description },
            { id: 'status', name: 'status', label: '状态', type: 'select', required: true, value: status,
              options: [
                  { value: 'ACTIVE', label: '活跃' },
        ...
        onSubmit: (formData) => {
            ...
            FunctionAPI.update(functionId, functionEntity)
                .then(() => {
                    showToast('功能更新成功', 'success');
                    loadFunctions();
                })
                .catch(error => {
...
// 删除功能
function deleteFunction(functionId, functionName) {
    if (confirm(`确定要删除功能"${functionName}"吗？此操作不可恢复！`)) {
        FunctionAPI.delete(functionId)
            .then(() => {
                showToast('功能删除成功', 'success');
                loadFunctions();
            })
            .catch(error => {
...
// 初始化角色分配页面
async function initRoleAssignmentPage() {
    const roleSelect = document.getElementById('role-select-assignment');
    if (!roleSelect) return;
    
    try {
        const roles = await RoleAPI.getAll();
        roleSelect.innerHTML = '<option value="">请选择角色...</option>' +
            roles.map(role => `<option value="${role.roleId}">${role.roleName || role.roleId}</option>`).join('');
    } catch (error) {
        console.error('加载角色失败:', error);
    }
}
...
async function loadUsersByRole() {
    const roleSelect = document.getElementById('role-select-assignment');
    const tbody = document.getElementById('user-role-table-body');
    if (!roleSelect || !tbody) return;
    
    const roleId = roleSelect.value;
    if (!roleId) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align: center;">请先选择角色</td></tr>';
        return;
    }
    
    try {
        const users = await UserAPI.getByRoleId(roleId);
        if (users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align: center;">该角色下暂无用户</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = users.map(user => `
            <tr>
                ...
                <td>${user.realName || ''}</td>
                <td>${user.email || ''}</td>
                <td>${roleMap[user.roleID] || user.roleID || '未分配'}</td>
                <td>
                    <div class="btn-group">
                        <button class="btn" onclick="assignRoleToUser(${user.userID}, '${(user.userName || '').replace(/'/g, "\\'")}')">🔧 分配角色</button>
                    </div>
                </td>
            </tr>
        `).join('');
...
function assignRoleToUser(userId, userName) {
    RoleAPI.getAll().then(roles => {
        Modal.form({
            title: `🔧 为用户"${userName}"分配角色`,
            fields: [
                { id: 'roleId', name: 'roleId', label: '选择角色', type: 'select', required: true,
                  options: roles.map(r => ({ value: r.roleId, label: r.roleName || r.roleId }))}
            ],
            submitText: '分配',
            onSubmit: (formData) => {
                UserAPI.assignRole(userId, formData.roleId)
                    .then(() => {
...
// 初始化权限分配页面
async function initPermissionAssignmentPage() {
    const roleSelect = document.getElementById('role-select-permission');
    if (!roleSelect) return;
...
async function loadPermissionsByRole() {
    const roleSelect = document.getElementById('role-select-permission');
    const area = document.getElementById('permission-assignment-area');
    if (!roleSelect || !area) return;
    
    const roleId = roleSelect.value;
    if (!roleId) {
        area.innerHTML = '<p style="text-align: center; color: #999;">请先选择角色</p>';
        return;
    }
    ...
    try {
        ...
        const functions = await FunctionAPI.getAll();
        // 获取该角色已有的权限
        const permissions = await PermissionAPI.getByRoleId(roleId);
        
        // 创建权限映射
        const permissionMap = {};
        permissions.forEach(p => {
            permissionMap[p.functionId] = p;
        });
        
        // 按子系统分组
        const subsystemMap = {};
        functions.forEach(func => {
            if (!subsystemMap[func.subsystem]) {
                subsystemMap[func.subsystem] = [];
            }
            subsystemMap[func.subsystem].push(func);
        });
        
        let html = '<div style="margin-bottom: 20px;"><button class="btn btn-accent" onclick="savePermissions()">💾 保存权限分配</button></div>';
        html += '<div id="permission-checkboxes">';
        
        Object.keys(subsystemMap).forEach(subsystem => {
            html += `<div style="margin-bottom: 30px;"><h4 style="margin-bottom: 15px; color: #2c3e50;">${subsystem}</h4>`;
            html += '<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 15px;">';
            
            subsystemMap[subsystem].forEach(func => {
                const permission = permissionMap[func.functionId];
                const isGranted = permission && permission.granted;
                
                html += `
                    <div style="border: 1px solid #ddd; padding: 15px; border-radius: 8px; background: ${isGranted ? '#f0f9ff' : '#fff'};">
                        <label style="display: flex; align-items: center; cursor: pointer;">
                            ...
                            <div>
                                <div style="font-weight: bold; margin-bottom: 5px;">${func.functionName || func.functionId}</div>
                                <div style="font-size: 0.9em; color: #666;">${func.description || func.functionPath || ''}</div>
                            </div>
                        </label>
                    </div>
                `;
        ...
        area.innerHTML = '<p style="text-align: center; color: red;">加载失败，请检查后端服务</p>';
    }
}
...
async function savePermissions() {
    ...
    if (!roleSelect || !checkboxes.length) {
        showToast('请先选择角色并加载权限', 'warning');
        return;
    }
    ...
    checkboxes.forEach(checkbox => {
        const functionId = parseInt(checkbox.dataset.functionId);
    ...
    try {
        await PermissionAPI.batchUpdate(roleId, permissions);
        showToast('权限分配保存成功', 'success');
        loadPermissionsByRole(); // 重新加载以刷新显示
    } catch (error) {
...
// 加载权限分配（刷新）
function loadPermissionAssignments() {
